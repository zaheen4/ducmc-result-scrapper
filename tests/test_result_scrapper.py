"""Tests for result_scrapper.py — pure functions, config/progress, sheet ops, selenium."""
...
class TestConfig:
    def test_load_config_defaults(self, tmp_path):
        with patch('result_scrapper.os.path.exists', return_value=False), \
             patch('result_scrapper.DATA_DIR', str(tmp_path)):
            config = load_config()
    ...
    def test_load_config_from_file(self, tmp_path):
        ...
        with patch('result_scrapper.DATA_DIR', str(tmp_path)):
            config = load_config()
    ...
    def test_load_config_partial_merges_defaults(self):
        cfg = {"exam": "Some Exam"}
        with patch('result_scrapper.os.path.exists', return_value=True), \
...
class TestProgress:
    def test_save_and_load_roundtrip(self, tmp_path):
        ...
        with patch('result_scrapper.PROGRESS_FILE', str(progress_file)):
...
class TestScrapeStudentResult:
    ...
    @patch('result_scrapper.FORM_DATA', {'program': 'prog', 'session': 'sess', 'exam': EXAM_TEXT})
    @patch('result_scrapper.Select')
    ...
    @patch('result_scrapper.time.sleep')
    @patch('result_scrapper.FORM_DATA', {'program': 'prog', 'session': 'sess', 'exam': 'Wrong Exam Name'})
    @patch('result_scrapper.Select')
...
class TestSelectExam:
    @patch('result_scrapper.time.sleep')
    @patch('result_scrapper.FORM_DATA', {'program': 'prog', 'session': 'sess', 'exam': ''})
    @patch('result_scrapper.Select')
    def test_no_exams_found(self, MockSelect, mock_sleep):
        ...
        mock_select.options = []
        MockSelect.return_value = mock_select
        with patch('result_scrapper.initialize_webdriver', return_value=mock_driver), \
    ...
    def test_skips_when_exam_already_set(self):
        ...
        with patch('result_scrapper.load_config', mock_load):
    ...
    def test_force_overrides_existing_exam(self):
        ...
        with patch('result_scrapper.initialize_webdriver', return_value=mock_driver), \
             patch('result_scrapper.WebDriverWait', return_value=mock_wait), \
             patch('result_scrapper.Select', return_value=mock_select), \
             patch('result_scrapper.time.sleep'), \
             patch('result_scrapper.load_config', mock_load), \
             patch('result_scrapper.save_config', mock_save), \
             patch.dict('sys.modules', {'InquirerPy': MagicMock(prompt=mock_prompt), 'InquirerPy.separator': MagicMock()}):
...
class TestTsLogging:
    def test_ts_writes_to_log_file(self):
        ...
        with patch('result_scrapper.LOG_FILE', mock_file):
            ts("test message")
        mock_file.seek(0)
...
class TestRequestDelay:
    def test_config_defaults_include_request_delay(self):
        with patch('result_scrapper.os.path.exists', return_value=False), \
             patch('result_scrapper.DATA_DIR', '/tmp'):
            config = load_config()
